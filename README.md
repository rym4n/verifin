...
- **WebDAV 自动双向同步**（可选）：与自动备份互斥，开启后以每台设备独立、不可变的完整 JSON 快照同步结构化数据，在本地按实体版本 merge；正常一轮只需列出根目录、拉取有更新设备的快照，并在本机有变更时上传一个快照，不再逐条上传账目。附件使用独立内容寻址 blob。多设备修改同一交易时检测冲突并进入冲突决议页手动选择保留哪一方；同步为**最终前台一致性**（eventual foreground consistency），不是实时云协作——打开应用或回前台时才同步，无后台推送或轮询；
...
- Android **桌面小组件**提供快速记账、预算进度、收支趋势、净资产四个固定模板，跟随当前账本；应用内预览直接展示桌面原生布局的渲染结果。另有下拉快捷开关「快速记账」与每日**记账提醒**通知；
...
| 云备份与同步 | `dart:io HttpClient` 手写 WebDAV 客户端；普通备份使用 PUT / GET / PROPFIND / MKCOL，双向同步 v2 使用根目录 PROPFIND 与不可变 JSON/blob GET / PUT |
