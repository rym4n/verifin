...
- Android 系统 Auto Backup 已显式关闭，账目和本机凭据不会绕过应用内备份设置进入系统云备份；换机请使用 Veri Fin 的导出/恢复或 WebDAV；
- 备份体系：手动 / 自动备份到本地目录（SAF）、**AES-GCM 加密**、**WebDAV 云备份**、zip 打包附件；
- **WebDAV 自动双向同步**（可选）：与自动备份互斥，开启后应用前台时自动增量同步账本数据到 WebDAV；多设备修改同一交易时检测冲突并进入冲突决议页手动选择保留哪一方；同步为**最终前台一致性**（eventual foreground consistency），不是实时云协作——打开应用或回前台时才同步，无后台推送或轮询；
